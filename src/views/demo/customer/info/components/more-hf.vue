    ...
    <Modal :mask-closable="false" :destroy-on-close="true" :title="drawerInfo.title" @cancel="drawerOnClose" @ok="submit"
      ...
      <Form :labelCol="{ span: 4 }" ref="formRef" :model="drawerInfo.item">
        <FormItem label="客户姓名" name="customerId" :rules="{
          ...
          <Select :disabled="drawerInfo.type !== 'add'" show-search :filter-option="filterOption" placeholder="请选择"
            allow-clear v-model:value="drawerInfo.item.customerId">
            <SelectOption v-for="item of dataSource" :value="item.id" :key="`${item.name}_${item.id}`">{{ item.name }}
            </SelectOption>
          </Select>
        </FormItem>

        <FormItem label="回访时间" name="visitTime" :rules="{
          required: true,
          message: '请选择回访时间',
          trigger: 'change',
        }">
          <DatePicker :disabled="drawerInfo.type === 'scan'" allow-clear placeholder="请选择"
            v-model:value="drawerInfo.item.visitTime" />
        </FormItem>

        <FormItem label="回访类型" name="type" :rules="{
          required: true,
          message: '请选择回访类型',
          trigger: 'change',
        }">
          <Select :disabled="drawerInfo.type === 'scan'" placeholder="请选择" allow-clear
        ...
        <FormItem label="内容简述" name="visitContent" :rules="{
          required: true,
          message: '内容简述',
          trigger: 'change',
        }">
