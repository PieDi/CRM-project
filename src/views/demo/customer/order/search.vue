  ...
  <PageWrapper title="客户订单管理" :back-show="showGoBack">
    <div :style="{ display: 'flex', justifyContent: 'space-between' }">
      <div :style="{ display: 'flex' }">
        <FormItem label="客户姓名">
          <Input placeholder="请输入" allowClear :style="{ width: '150px' }" v-model:value="searchInfo.customerName" />
        </FormItem>
    ...
    <Table :columns="columns" :dataSource="pageInfo.dataSource" :canResize="false" :striped="false" :bordered="true"
    ...
    <Modal :mask-closable="false" :destroy-on-close="true" :title="drawerInfo.title" @cancel="drawerOnClose" @ok="submit"
      ...
      <Form :labelCol="{ span: 4 }" ref="formRef" :model="drawerInfo.item">
        <FormItem label="客户姓名" name="customerId" :rules="{
          required: true,
          message: '请选择客户姓名',
          trigger: 'change',
        }">
          <Select show-search :filter-option="filterOption" :disabled="drawerInfo.type !== 'add'" placeholder="请选择"
            v-model:value="drawerInfo.item.customerId">
            <SelectOption v-for="item of cDataSource" :value="item.id" :key="`${item.name}_${item.id}`">{{ item.name }}
            </SelectOption>
          </Select>
        </FormItem>

        <FormItem label="订单名称" name="orderName" :rules="{
          required: true,
          message: '请输入订单名称',
          trigger: 'change',
        }">
        ...
        <FormItem label="下单时间" name="orderDate" :rules="{
          required: true,
          message: '请选择订单时间',
          trigger: 'change',
        }">
          <DatePicker :disabled="drawerInfo.type === 'scan'" showTime placeholder="请选择"
        ...
        <FormItem label="订单产品" style="margin-bottom: 0">
          ...
          <Space v-for="(p, i) of drawerInfo.item.products" align="start">
            <FormItem label="产品名称" :name="['products', i, 'productId']" :rules="{
              required: true,
              message: '请选择产品名称',
              trigger: 'change',
            }">
        ...
        <FormItem label="订单金额" name="totalPrice" :rules="{
          required: true,
          message: '请输入订单金额',
          trigger: 'change',
        }">
        ...
        <FormItem label="负责人" name="responsiblePerson" :rules="{
          required: true,
          message: '请输入负责人',
          trigger: 'change',
        }">
