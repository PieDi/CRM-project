<template>
  <Modal :mask-closable="false" :destroy-on-close="true" :title="drawerInfo.title" @cancel="drawerOnClose" @ok="submit"
    :visible="drawerInfo.visible" width="60%">
    <Form :labelCol="{ span: 4 }">
      <FormItem label="客户姓名">
        <Select :disabled="drawerInfo.type !== 'add'" placeholder="请选择" @change="customerOnChange"
      ...
      <template v-if="currentCustomer?.id">
        ...
        <FormItem label="证件号码">
          <Input placeholder="请输入" allowClear disabled v-model:value="currentCustomer.documentNumber" />
        </FormItem>
      </template>

      <FormItem label="就诊医院">
        <Input placeholder="请输入" allowClear v-model:value="mInfo.hospitalName" :disabled="drawerInfo.type === 'scan'" />
      </FormItem>

      <FormItem label="科室">
        <Input placeholder="请输入" allowClear v-model:value="mInfo.departmentName" :disabled="drawerInfo.type === 'scan'" />
      </FormItem>
      <FormItem label="疾病名称">
        <Input placeholder="请输入" allowClear v-model:value="mInfo.diseaseName" :disabled="drawerInfo.type === 'scan'" />
      </FormItem>

      <FormItem label="会诊日期">
        <DatePicker :show-time="true" v-model:value="mInfo.visitDate" :disabled="drawerInfo.type === 'scan'" />
      </FormItem>

      <FormItem label="附件">
        <Upload :file-list="fileList" :customRequest="uploadAction" :disabled="drawerInfo.type === 'scan'">
